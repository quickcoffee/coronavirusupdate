...
<!-- badges: start -->
[![R-CMD-check](https://github.com/quickcoffee/coronavirusupdate/workflows/R-CMD-check/badge.svg)](https://github.com/quickcoffee/coronavirusupdate/actions)
...
In this repo every Wednesday a GitHub action is triggered to get the newest transcripts from NDR.de (as long as the website doesn't change).  
For all non-R users you can also find the data as parquet file in `data/coronavirusupdate_transcripts.parquet`.
...
paragraph. Currently this includes `r max(coronavirusupdate_transcripts$episode_no)` 
episodes with the following structure:
...
```{r speaker-share, message=FALSE, warning=FALSE}
library(ggplot2)
...
Or one can create a tf-idf analysis to find out which words are "unique" to each speaker:
